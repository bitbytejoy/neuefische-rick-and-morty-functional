...
function filterAlive (characters: Character[]): Character[] {
  return characters.filter(({ status }) => status === "Alive");
}

function mapName (characters: Character[]): string[] {
  return characters.map(({name}) => name);
}

function mapNameOrigin (characters: Character[]): {name: string, origin: string}[] {
  return characters.map(({name, origin: {name: originName}}) => ({name, origin: originName}));
}

function richAndMorty () {
  const characters: RickAndMortyCharacterResponse =
    {
      "info": {
        "count": 826,
        "pages": 42,
  ...
  console.log("Alive", filterAlive(characters.results));
  console.log("Names", mapName(characters.results));
  console.log("Name & Origin", mapNameOrigin(characters.results));
}
